...
Usage:  sign_target_files_apks [flags] input_target_files output_target_files
  ...
  --bump_date_and_version <increment>
      ...
      by the given amount (defaults to 1)
...
def RewriteProps(data):
  ...
  for line in data.split("\n"):
    ...
    if line and line[0] != '#' and "=" in line:
      ...
      elif key.startswith("ro.") and key.endswith(".build.version.incremental") and
              OPTIONS.bump_date_and_version:
        value = str(int(value) + OPTIONS.bump_date_and_version)
      line = key + "=" + value
...
def main(argv):
  ...
  if OPTIONS.otatest and not OPTIONS.bump_date_and_version:
